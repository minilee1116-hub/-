...
import { Check, ArrowRight, ArrowLeft, Send, Sparkles, AlertCircle, Lock } from 'lucide-react';
import { Consultation } from '../types';
import { submitConsultation, fetchSettings } from '../lib/content';
...
export default function ConsultingForm({ onSuccess }: ConsultingFormProps) {
  ...
  // 점검 모드 상태 (null=불러오는 중). 설정값 'off'면 폼 노출, 그 외(기본)는 점검 메시지.
  const [maintenance, setMaintenance] = useState<boolean | null>(null);
  useEffect(() => {
    fetchSettings().then((s) => {
      setMaintenance((s.consulting_maintenance ?? 'on') !== 'off');
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await submitConsultation(newConsultation);
  ...
  if (maintenance === null) {
    return (
      <div className="w-full max-w-2xl mx-auto rounded-3xl bg-neutral-950/80 border border-neutral-900 p-12 text-center text-neutral-500">
        불러오는 중...
      </div>
    );
  }
